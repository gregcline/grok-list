...
* [`src/cljs/gregcline\grok_list_ui/`](src/cljs/gregcline\grok_list_ui/): SPA source files (ClojureScript,
[re-frame](https://github.com/Day8/re-frame))
  - [`core.cljs`](src/cljs/gregcline\grok_list_ui/core.cljs): contains the SPA entry point, `init`
...
7. Clone this repo and open a terminal in the `grok_list_ui` project root directory
...
The `debug?` variable in [`config.cljs`](src/gregcline\grok_list_ui/config.cljs) defaults to `true` in
[`dev`](#running-the-app) builds, and `false` in [`prod`](#production) builds.

Use `debug?` for logging or other tasks that should run only on `dev` builds:

```clj
(ns gregcline.grok-list-ui.example
  (:require [gregcline.grok-list-ui.config :as config])
